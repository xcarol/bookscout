...
async function scrapeEBiblioCat(isbn, region) {
  ...
  try {
    ...
    try {
      await page.waitForFunction(() => !document.querySelector('app-waiting-button'), {
        timeout: 15000,
      });
    } catch {
    ...
    if (allBtnText.includes('reserv') || allBtnText.includes('hold')) {
      ...
      allBtnText.includes('prestar') ||
      allBtnText.includes('llegir') ||
      allBtnText.includes('borrow') ||
      allBtnText.includes('read')
    ) {
