...
async function buildBookEdition(isbn) {
  ...
  const publisher =
    ...
    (olData?.publishers?.length > 0 ? olData.publishers[0]?.name : null) ||
    null;
  const publishedDate = volumeInfo.publishedDate || olData?.publish_date || null;
  const language =
    volumeInfo.language ||
    (olData?.languages?.length > 0 ? olData.languages[0]?.name : null) ||
    null;
  ...
  if (Array.isArray(olData?.series) && olData.series.length > 0) {
    const seriesName =
      typeof olData.series[0] === 'string' ? olData.series[0] : olData.series[0]?.name;
    if (seriesName) {
      series = {
        name: seriesName,
        position: null,
  ...
  let format = 'UNKNOWN';
  if (
    googleData?.saleInfo?.isEbook ||
    olData?.physical_format?.toLowerCase().includes('ebook') ||
    olData?.physical_format?.toLowerCase().includes('digital')
  ) {
    format = 'DIGITAL';
