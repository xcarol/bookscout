...
async function checkAvailability(isbn, country = 'GLOBAL', region = null) {
  ...
  const bookFormat = editionDoc?.exists
    ? editionDoc.data().format || FORMATS.UNKNOWN
    : FORMATS.UNKNOWN;
    ...
    if (result.status === 'fulfilled' && result.value) {
      if (result.value.error) {
        if (result.value.errorType === ERROR_TYPES.NOT_FOUND) {
          ...
          console.error(
            `[ERROR] ${result.value.providerName}: Error - ${result.value.errorMessage}`,
          );
      ...
      if (
        bookFormat !== FORMATS.UNKNOWN &&
        providerFormat !== FORMATS.UNKNOWN &&
        bookFormat !== providerFormat
      ) {
        console.info(
          `[FILTER] Skipping provider ${result.value.providerName} for ISBN ${isbn} because formats do not match (${providerFormat} vs ${bookFormat})`,
        );
        return;
      }
      ...
      if (
        ...
        Object.keys(result.value.metadata).length > 0
      ) {
        const meta = result.value.metadata;
        const updateDoc = { updatedAt: new Date().toISOString() };

        if (meta.title) updateDoc.title = meta.title;
        if (meta.description || meta.synopsis)
          updateDoc.description = meta.description || meta.synopsis;
